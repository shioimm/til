...
def self.tcp(host,
  ...
  # 接続できたソケット、未接続のSocket、未接続のSocketと接続対象のAddrinfoのハッシュ、エラー
  ...
  # 接続済みのソケットを取得できておらず、未接続のSocketがある場合
  ...
  # ここまでで接続可能なソケットを得られていない場合のエラー処理
  unless ret
    if last_error
      raise last_error
...
```ruby
# socket_list -> 未接続のSocket
# addrinfos   -> 未接続のSocketと接続対象のAddrinfoのハッシュ
...
- メインスレッドで接続試行を実施
  - 別スレッドでのアドレス解決 (Resolution Delay) を待ち合わせる
  - `pipe_reads`がread可能になったら解決できたアドレスのから接続試行するアドレスを選択
