...
def self.tcp(host,
  ...
  # 接続できたソケット、wait_writableなSocket、wait_writableなSocketと接続対象のAddrinfoのハッシュ 、エラー
  ...
  # 接続済みのソケットを取得できておらず、wait_writableなSocketがある場合
...
def self.make_connection_attempts(pipe_read,
  ...
  # getaddrinfoが完了していない場合、もしくはアドレス取得済みの場合
  while !getaddrinfo_v6_done || !getaddrinfo_v4_done || !ai_list_v6.empty? || !ai_list_v4.empty?
    # 2周目以降あり得る値:
    #   socket_list                -> wait_writableなSocket
    #   addrinfos                  -> { <wait_writableなSocket> => <接続対象のAddrinfo> }
