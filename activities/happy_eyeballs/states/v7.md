...
loop do
  ...
  (4) if Resolution waiting pipe: any
    ...
    # Connection attempt delay時間内の可能性あり。Resolution delay時間内の可能性はない
    ...
    while Resolved family, Resolved addrinfos = Hostname resolution queue.pop
      ...
      # TODO 要修正: この条件だと、Queueの中に[IPv4 Addrinfo, IPv6 Addrinfo]の順で結果が格納されていた場合に
      # 誤ってResolution Delayを設定してしまう
      if Resolved family: IPv4 && Resolved addrinfos: IPv6 not yet resolved
        # Connection attempt delay時間内の場合はここは通らない
