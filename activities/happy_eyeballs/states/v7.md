...
- user_specified_resolv_timout_at # 名前解決開始時刻 + resolv_timeout
...
loop do
  ...
  (3) if (connect_timeout時間超過 || resolv_timeout時間超過) && Resolution waiting pipe: empty
  ...
  (5) if Resolved addrinfos: any &&
         (connection_attempt_delay_expires_at: 時間超過 ||
         (IPv6アドレス解決済み || resolution_delay_expires_at: 時間超過))
    ...
    if Resolved addrinfos: any || Connecting sockets: any || Hostname resolution queue: opened
      connect_nonblock
