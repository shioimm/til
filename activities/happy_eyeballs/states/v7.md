...
- Connecting sockets (Writable socket)
  - any / empty
- Writable sockets
  - any / empty
- Connected socket
...
loop do
  ...
  if (1) イベント発生: Writable sockets: any
  ...
  if (2) Connected socket: any
    return Connected socket
  end

  if (3) (connect_timeout時間超過) ||
     (resolv_tiemout時間超過 &&
  ...
  if (4) イベント発生: Resolution waiting pipe: any
    ...
    case Resolved family: IPv4 && Resolved addrinfos: IPv6 not yet resolved
      ...
      # このループでは接続試行を開始しない。次のループのselectでEnds atを待った結果、
      # 名前解決が発生した場合は (4) -> (5)
      # 名前解決が発生しなかった場合は (5)
      # その前にresolv_timeout時間超過が発生した場合は (3) が実行される
  end

  if (5) Resolved addrinfos: any && Ends at: 時間超過
    # Ends at = Connection attempt delay
    ...
    if Resolved addrinfos: empty && Connecting sockets: empty && Hostname resolution queue: closed
      ...
      case 接続中
        ...
        接続を待つために次のループへ進む # Hostname resolution queue: opened の場合は次のループで名前解決も待つ
  ...
  if (6) Connected socket: any &&
     Resolved addrinfos: empty &&
     Connecting sockets: empty &&
     Hostname resolution queue: closed
    raise Last error
  end
end
```
