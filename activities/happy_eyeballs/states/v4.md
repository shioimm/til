...
```
扱うリソースなど
- Hostname resolution rpipe ... 名前解決を通知するpipe (v4解決、v6解決、エラー発生のいずれかを通知する)
...
case v6c, v4c, v46c
  ...
  do
    Selectable addrinfos.get
    Connecting socket = Socket.new

    if Local addrinfos.any?
      if Connecting socketにbindできるLocal addrinfoがある
