...
case start
  ...
  do
    ...
    while 次のstateが決定するまで
      ...
      case v6名前解決
        Selectable addrinfo = Hostname resolution queue.get
...
case v6c, v4c, v46c
  ...
  condition
    Local addrinfosの状態:
      - Local addrinfos: any
        - 接続先と同じアドレスファミリのLocal addrinfoがある場合
        - 接続先と同じアドレスファミリのLocal addrinfoがない場合
    ...
    case Connectable sockets: any
      while (Connectable socket = Connectable sockets.pop)
        ...
        case Errno::EISCONN以外のエラー
          ...
          case Connecting sockets: any && Selectable addrinfos: any && Hostname resolution queue: any
            # 次のループでConnecting socketsの接続を待つ。さらに次のループでv46cに遷移するかも。
            -> v46w

          case Connecting sockets: any && Selectable addrinfos: any && Hostname resolution queue: empty
            # 次のループでConnecting socketsの接続を待つ。さらに次のループでv46cに遷移するかも。
            -> v46w

          case Connecting sockets: any && Selectable addrinfos: empty && Hostname resolution queue: any
            # 次のループでConnecting socketsの接続を待つ。さらに次のループで再びv46wに遷移した後、その次のループでv46cに遷移するかも。
            -> v46w

          case Connecting sockets: any && Selectable addrinfos: empty && Hostname resolution queue: empty
            # 次のループでConnecting socketsの接続を待つ。あとは接続を待つことしかできない
            Connection attempt delay expires at = nil
            -> v46w

          case Connecting sockets: empty && Selectable addrinfos: any && Hostname resolution queue: any
            # 次のループでConnection Attempt Delay タイムアウトを待つ。さらに次のループでv46cに遷移するかも。
            -> v46w

          case Connecting sockets: empty && Selectable addrinfos: any && Hostname resolution queue: empty
            # 次のループでConnection Attempt Delay タイムアウトを待つ。さらに次のループでv46cに遷移するかも。
            -> v46w

          case Connecting sockets: empty && Selectable addrinfos: empty && Hostname resolution queue: any
            # あとは名前解決を待つことしかできない
            Connection attempt delay expires at = nil
            -> v46w

          case Connecting sockets: empty && Selectable addrinfos: empty && Hostname resolution queue: empty
