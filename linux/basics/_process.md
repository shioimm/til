...
## プロセスの終了
- プロセスが`exit`を呼び出し、親プロセスがその終了状態を捕捉すると、プロセスは終了する
  - 親プロセスは`wait`を呼び出すことで子プロセスの終了状態を捕捉する
  - 親プロセスが子プロセスの終了状態を捕捉すると、カーネルは子プロセスを消滅させる
  - 親プロセスが終了状態を捕捉できない場合、子プロセスはゾンビ化する
- プロセスが終了する時そのプロセスが使っていた資源は自動的に解放され
  カーネルによって回収される
