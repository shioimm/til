...
- ファイルシステムは自身が持つ全てのファイルのデータ領域とi-node領域を管理する
  - データ領域    - ファイルの実データを格納する領域
  - i-node領域   - i-nodeの配列を格納する領域
    - i-node番号 - 配列の添字
    - i-node     - 各ファイルの属性情報を格納する構造体
...
シリンダグループ(0 + n)
    ...
    |- データブロック0    (シンボリックリンクのためのデータ)
        |- リンクしたい実データのファイル名へのリンクを持つi-node番号
    |- データブロック0 + n(シンボリックリンクのためのデータ)
        |- リンクしたい実データのファイル名へのリンクを持つi-node番号
    |- ...
```

- 複数のディレクトリエントリが同一のi-nodeエントリを指す場合がある(リンク)
  - 各i-nodeは当該i-nodeを指すディレクトリエントリの個数を格納したリンクカウントを持つ
- ディレクトリエントリ内のi-node番号は同じファイルシステム内のi-nodeを指す
