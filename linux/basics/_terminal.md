...
## カノニカルモード
- `read`を発行すると端末ドライバは一行全体を返す
  - 指定バイト数だけ読み取ると`read`から戻る
    - 次の`read`は直前の`read`で止まったところから始まる
  - 行区切りに出会うと`read`から戻る
  - シグナルを捕捉して自動的に再始動しないと`read`から戻る

## 非カノニカルモード
- `termios`構造体のフィールド`c_lflag`のフラグ`ICANON`をオフにすることで指定できる
- 入力データは行に組み上げられない
  - 指定バイト数だけ読み取ると`read`から戻る
  - 指定時間を経過すると`read`から戻る
...
- `tty(1)`(`/dev/tty`) - 端末をデバイスファイルとして抽象化したものを取得する
  - `ctermid(3)` - 制御端末名の判定
  - `isatty(3)` - 端末装置を参照するファイルディスクリプタへ真を返す
  - `ttyname(3)` - ファイルディスクリプタにオープンされた端末装置のパス名を返す
...
## 端末ウィンドウサイズ
- 端末の現在のウィンドウサイズを追跡し、サイズが変化するとフォアグラウンドプロセスグループが通知する
...
- ボーレート = ビット / 秒
  - 一秒間の変調回数
- ハードウェアが対応する場合入出力それぞれにことなる速度値を指定することができる
  - `cfsetispeed(3)` / `cfsetospeed(3)`
- 端末の入出力の各ボーレートは端末装置の`termios`構造体に格納される

## ライン制御関数群
- 端末装置に対する信号制御機能を提供する
  - `tcdrain(3)` - 全ての出力が送出されるのを待つ
  - `tcflow(3)` -入出力フロー制御の制御
  - `tcfluch(3)` - 入力バッファや出力バッファを捨てる
  - `tcsendbreak(3)` - 指定期間だけビット0の列を創出し続ける
