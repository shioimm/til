...
- `sem_open(3)` - 新規セマフォの作成または既存のセマフォのハンドルを返す
  - 作成した場合は同時に初期化を行う
- `sem_post(3)` - セマフォ値をインクリメントする
- `sem_wait(3)` - セマフォ値をデクリメントする
- `sem_getvalue(3)` - セマフォの現在値を取得する
- `sem_close(3)` - セマフォをクローズする
- `sem_unlink(3)` - セマフォの名前を削除し、削除マークを行う
  - 全プロセスがクローズすると実際に削除される

### 無名
#### API
- `sem_init(3)` - 無名セマフォの作成
- `sem_destroy(3)` - 無名セマフォの破棄
