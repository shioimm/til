...
{
  struct RClass *cls = mrb_define_class(mrb, "Hello", mrb->object_class);
  mrb_define_method(mrb, cls, "initialize", mrb_mruby_hello_initialize, MRB_ARGS_REQ(2));
  mrb_define_method(mrb, cls, "greeting", mrb_mruby_greeting, MRB_ARGS_NONE());
}
