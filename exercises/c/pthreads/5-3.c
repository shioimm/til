// Linuxとpthreadsによるマルチスレッドプログラミング入門 P150
...
void BirdWaitForSetDestination(Bird *bird)
{
  pthread_mutex_lock(&bird->mutex);

  if (pthread_cond_wait(&bird->cond, &bird->mutex) != 0) {
...
void *doMove(void *arg)
{
  ...
  while (!stopRequest) {
    bird->busy = 0;
    BirdWaitForSetDestination(bird);
