...
- ワンショットで実行する前提の実装なので、初期設定はクラスが持ち、実行ごとに値を上書きできるようになっている
  - 初期設定をして`HTTParty.get`を呼び出す
  - 初期設定せずに`HTTParty.get`にプロキシ情報を渡して呼び出す
  - 初期設定をして`HTTParty.get`にプロキシ情報を渡して呼び出す (上書き)
- 環境変数から取得するあたりはnet-httpに任せている
