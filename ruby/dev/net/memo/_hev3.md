...
- 基盤ネットワークが IPv6-only
- 名前解決結果がAレコードのみ
- そのIPv4 アドレスがVPN のルーティング範囲外
- その名前がVPN (scoped DNS) で解決された
  - 企業リゾルバはNAT64 prefixを知らず自分で合成できない
  - 基盤ネットワークのDNS64は内部名を解決できず合成できない
```
...
- やる...?
  - VPN環境の判定
  - VPN (scoped DNS) で名前解決されたことの判定
  - 宛先IPv4アドレスがVPN のルーティング範囲外であることの判定
- UDP `connect(2)`による経路プローブを行う前提でIPv6-only / mostly対応にまとめていいのかも
...
- HTTP/3非対応・HTTP/2対応クライアントの場合:
  - `alpn="h3"` + `no-default-alpn` (HTTP/3専用) を広告しているエンドポイントへ接続できない
  - `alpn="h3,h2"`は"h2"として扱い、優先度のみを考慮する
...
#### アドレスリストが複数グループある場合の接続試行
- Step 1の最優先グループ内のStep 2先頭グループから順に全Step 2グループ内の全アドレスを接続試行する
- Step 1の他グループへのフォールバックは実装定義

#### ALPNを利用する場合の接続試行
- TLSハンドシェイクの際、ClientHelloに含めるALPNリストをSVCB ALPN setに絞る必要あり
- QUIC (あれば) とTCPは別々の接続試行として扱い、それぞれに適切なalpn値を参照する
...
- IPv6-only / IPv6-mostlyネットワークのためのNAT64 prefixの取得とIPv6アドレス合成
  - 検証環境の構築
- DNS問い合わせ時、DNS再帰リゾルバのアドレスリストにIPv6アドレスがある場合は優先的にそれを利用する
  - /etc/resolv.confだけを確認すれば良いわけではなく、ポータビリティの問題がありそう。やる...?
...
- 取得しているアドレスに対してアドレスリストのグルーピングとソートを行う
  ...
  - (5.2)
    - サービス単位でそのターゲット名のA / AAAA (あるいはHTTPSレコード内のipv4hint / ipv6hint) をグルーピング
      ...
      - h3専用エンドポイントは除外
      - `no-default-alpn`がついていない`alpn={h3}`はHTTP/1.1接続グループにまとめる
    - グループ同士を優先度でソート
    - ターゲットにHTTPS RRが設定されていない、あるいは否定応答を取得した、あるいは遅延のうえRD経過した場合は
      全アドレスを1グループ・同一優先度として扱う
      - RD経過のうえ、接続試行開始後にHTTPS RRを受信した場合にどうするかは実装依存っぽい
