...
# 複数のジェネレータを組み合わせることが可能
for x <- [1, 2], y <- [3, 4], do: x * y # => [3, 4, 6, 8]

# 複数のジェネレータとフィルタを組み合わせることが可能
for x <- [1, 2], y <- [3, 4], x > 1 and y > 3 do: x * y # => [8]
```
